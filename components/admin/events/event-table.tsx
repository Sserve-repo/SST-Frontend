...
export function EventTable({ events }: EventTableProps) {
  ...
  return (
    <>
              ...
              <TableRow key={event.id}>
                ...
                <TableCell>
                  <Badge
                    variant="secondary"
                    className={cn(
                      event.location === "virtual" && "bg-blue-100 text-blue-600",
                      event.location === "in_person" && "bg-green-100 text-green-600",
                      event.location === "hybrid" && "bg-purple-100 text-purple-600",
                    )}
                  >
                    {event.location}
                  </Badge>
                ...
                <TableCell>
                  <Button variant="ghost" size="sm" className="gap-2" onClick={() => setEventToView(event)}>
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => setEventToView(event)}>View Details</DropdownMenuItem>
      ...
      <EventDetailsDialog event={eventToView} onOpenChange={(open) => !open && setEventToView(null)} />
      <EditEventDialog event={eventToEdit} onOpenChange={(open) => !open && setEventToEdit(null)} />
      <DeleteEventDialog event={eventToDelete} onOpenChange={(open) => !open && setEventToDelete(null)} />
    </>
  )
}
