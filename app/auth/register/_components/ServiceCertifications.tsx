...
const ServiceCertifications: React.FC<ServiceCertificationsProps> = ({
  ...
  const handleFetchUser = async () => {
    ...
    if (response && response.ok) {
      ...
      const fetchedCat = data.data["Service Category Menu"];
      fetchedCat.map((service: any) => {
        if (service.id === service_category_id) {
          setSelectedCategory(service.name.toLowerCase());
          service["service_category_items"].map((item) => {
            if (item.id === service_category_item_id) {
              setSelectedSubCategory(item.name.toLowerCase());
              return;
            }
          });
        }
    ...
    <div className="flex flex-col gap-y-6">
      {certificationFields &&
        certificationFields.map((field, index) => {
          const isOptional = field.description
            .toLowerCase()
            .includes("optional");
