...
const promotionSchema = z.object({
  ...
  discount_value: z.number().min(0.01, "Discount value must be greater than 0"),
  start_date: z.string().min(1, "Start date is required"),
  end_date: z.string().min(1, "End date is required").refine((endDate, ctx) => {
...
export function EditPromotionDialog({
  ...
  const onSubmit = async (data: PromotionFormData) => {
    ...
    try {
      ...
      // Convert form data to FormData object for the parent component
      ...
      onSuccess(promotion.id as number, formData);
